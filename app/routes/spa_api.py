...
def _sample_dashboard() -> dict[str, Any]:
    ...
    return {
        "series": [
            {"name": entry["name"], "data": list(entry["data"])} for entry in _SAMPLE_SERIES
        ],
        "summary": {**_SAMPLE_SUMMARY, "highlights": list(_SAMPLE_SUMMARY["highlights"])},
    }
...
def fetch_dashboard(
    ...
    sentiment = _percent(metrics.get("avg_mood"))
    avg_stress = metrics.get("avg_stress")
    risk_snapshot = risk.latest_risk_snapshot(db, team)
    risk_level = (metrics.get("risk_level") or risk_snapshot.risk_level.value or "low").lower()
